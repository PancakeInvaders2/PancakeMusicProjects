...
export class Interval implements EqualsTrait {

	private static SUS_2_PRIORITY = 4;
	private static SUS_4_PRIORITY = 5;
	...
	private constructor(semitones: number,
		name: string,
		isThird: boolean,
		isFifth: boolean,
		orderingPriority: number,
	...
	private static semitonesToInterval: Map<number, Interval> = new Map([
	...
	static getIntervalBySemitones(semitones: number): Interval | undefined {
		return Interval.semitonesToInterval.get(semitones);
	}

	
	getName(intervals: Interval[] | null): string {
		if (intervals !== null) {
			if (
				this === Interval.MIN3 &&
	...
	getOrderingPriority(intervals: Interval[]): number {
		if (
			this === Interval.MAJ2 &&
